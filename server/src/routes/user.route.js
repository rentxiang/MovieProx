...
router.post(
    ...
    .exists().withMessage("comfirmPassword is required")
    .isLength({ min: 8}).withMessage("confirmPassword minimum 8 characters")
    .custom(( value, { req }) => {
        if (value !== req.body.password) throw new Error ("confirmPassword wrong")
        return true;
    }),
    body("displayName")
...
router.put(
    ...
    .isLength({ min: 8}).withMessage("password minimum 8 characters")
    .custom(( value, { req }) => {
        if (value !== req.body.newPassword) throw new Error ("confirmNewPassword wrong")
        return true;
    }),
    requestHandler.validate,
